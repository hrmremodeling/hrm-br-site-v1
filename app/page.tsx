...
function ImageTile({ src, alt, className = "" }: { src: string | null; alt: string; className?: string }) {
  if (!src) {
    return (
      <div className={`flex h-full w-full items-end rounded-2xl bg-gradient-to-br from-neutral-200 to-neutral-300 p-4 ${className}`.trim()}>
        <div className="rounded-xl bg-white/80 px-3 py-2 text-xs font-medium text-neutral-700 shadow-sm">{alt}</div>
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-neutral-50 text-neutral-900">
        ...
        <section className="relative">
          <div className="relative h-[560px] w-full overflow-hidden">
            <img src={projectImages.heroKitchen} alt="Kitchen remodel" className="absolute inset-0 h-full w-full object-cover" />
            ...
            <div className="relative mx-auto flex h-full max-w-7xl flex-col justify-center px-6 text-white">
              <div>
                <h1 className="max-w-3xl text-5xl font-bold leading-tight tracking-tight md:text-6xl">HRM Building &amp; Remodeling LLC</h1>
                <h2 className="mt-4 max-w-3xl text-3xl font-semibold leading-tight text-white/95 md:text-4xl">Kitchen, Bathroom, Basement &amp; Deck Specialists in Vernon, CT</h2>
              </div>

              <p className="mt-6 max-w-3xl text-lg leading-relaxed text-white/90">Kitchen, bathroom, basement, and deck specialists serving Vernon, CT and surrounding communities with over 25 years of experience delivering honest work, quality craftsmanship, and beautiful results.</p>
              <p className="mt-4 max-w-3xl text-base leading-7 text-white/80">We handle kitchen remodeling, bathroom renovations, basement finishing, deck construction, and window and door installation throughout the Vernon area. Every project is built with careful craftsmanship, clear communication, and attention to detail. Financing options are also available to help make your project more manageable.</p>
              ...
              <div className="mt-10 flex flex-wrap gap-5">
                ...
                <Button variant="secondary" className="rounded-xl border border-white px-6 py-5 text-base text-white hover:bg-white/10">View Our Projects</Button>
              </div>

              <div className="mt-10 flex flex-wrap gap-8 text-base text-white/90">
                {['Kitchens','Bathrooms','Basements','Decks','Windows & Doors'].map((item) => (
                  <div key={item} className="flex items-center gap-2"><CheckCircle2 size={16} /> {item}</div>
                ))}
              </div>
            </div>
          </div>
            ...
            <Card className="overflow-hidden rounded-[2rem] border-0 bg-white shadow-2xl shadow-black/10">
              <CardContent className="grid gap-6 p-6 md:grid-cols-[1.15fr_0.85fr] md:p-8">
                <div>
                  ...
                  <p className="mt-4 max-w-2xl text-sm leading-7 text-neutral-600">HRM Building &amp; Remodeling LLC has built a reputation in the Vernon area for honest work, clear communication, and quality craftsmanship. Every project is approached with attention to detail and a focus on creating spaces that are both functional and built to last for years.</p>
                </div>
        ...
        <section id="services" className="mx-auto max-w-7xl px-6 pb-20 pt-16 md:pt-20">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <p className="mt-4 text-neutral-600">With a quarter-century of expertise, HRM Building And Remodeling LLC is your trusted partner for home transformations in the Vernon area. Specializing in kitchen remodeling, bathroom renovations, basement finishing, decks, windows, and doors, our team brings visions to life with unparalleled skill. From initial design to final installation, we provide a seamless, full-service experience. Our commitment to honesty, integrity, and quality craftsmanship ensures every project is completed to the highest standard, delivering beautiful and lasting results.</p>
          </div>
              ...
              {featuredProjects.map((project, idx) => (
                <motion.div key={project.title} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.05, duration: 0.35 }}>
                  <Card className="overflow-hidden rounded-3xl border-0 bg-white shadow-md shadow-black/5">
                    <div className="aspect-[4/3] p-3"><ImageTile src={project.image} alt={project.title} /></div>
                    <CardContent className="p-6">
                      <Badge variant="secondary" className="rounded-full bg-neutral-100 text-neutral-700">{project.tag}</Badge>
              ...
              <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                {gallery.map((item) => (
                  <Card key={item.title} className="overflow-hidden rounded-3xl border-0 shadow-md shadow-black/5">
                    <div className="aspect-square p-3"><ImageTile src={item.image} alt={item.title} /></div>
              ...
              <Card className="rounded-[2rem] border-white/10 bg-white/5 text-white shadow-xl">
                <CardContent className="p-8">
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    ...
                    <Button variant="secondary" className="rounded-xl border border-white px-5 py-4 text-sm text-white hover:bg-white/10">Leave a Review</Button>
                  </div>
                </CardContent>
              </Card>
            </div>
